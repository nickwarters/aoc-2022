...
function solve(input: string): any {
    ...
    const diviser = monkeys.reduce((prev, next) => prev * next.test, 1)
    const calmDown = (value: number) => value % diviser
    const monkeyTurn = (m: Monkey): void => {
        for(let i = 0; i < m.items.length; i++){
            let newItem = calmDown(m.calculateWorry(m.items[i]))
            monkeys[newItem % m.test === 0 ? m.passTo : m.failTo].items.push(newItem)
            m.seen += 1
        }
        m.items = []
    } 
